...
caches:
  google_cache:
    cache:
      type: file
      directory: C:/mapproxy/cache_data/google
```
...
`.env.local` 설정:
```
NEXT_PUBLIC_MAP_TILE_URL=http://localhost:8080/wmts/google/webmercator/{z}/{x}/{y}.png
```

VM IP로 외부 접근 시:
```
NEXT_PUBLIC_MAP_TILE_URL=http://{VM_IP}:8080/wmts/google/webmercator/{z}/{x}/{y}.png
```
